...
typedef struct {
    const char *start;
    const char *fmt;
    json_token_t prev_token;
    json_token_t token;
    json_token_t next_token;
    json_error_t *error;
    size_t flags;
    int line;
    int column;
                         ...
                         size_t flags, const char *fmt)
{
    ...
    s->flags = flags;
    s->fmt = s->start = fmt;
    memset(&s->prev_token, 0, sizeof(json_token_t));
    memset(&s->token, 0, sizeof(json_token_t));
    memset(&s->next_token, 0, sizeof(json_token_t));
    s->line = 1;
    s->column = 0;
    s->pos = 0;
}
